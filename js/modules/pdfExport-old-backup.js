...
const PDFExport = (() => {
  ...
  const init = () => {
    ...
    if (typeof window.jspdf !== "undefined") {
      jsPDFLoaded = true;
      console.log("PDFExport: jsPDF library detected");
    } else {
      console.warn(
        "PDFExport: jsPDF library not loaded, will use text export fallback"
      );
    }
  ...
  const generatePDFBytes = (workout) => {
    ...
    const exerciseLines = workout.map((exercise, index) => {
      ...
      const muscleGroup = exercise.muscleGroup
        ? `(${
            ExerciseDatabase?.getMuscleGroupLabel?.(exercise.muscleGroup) ||
  ...
  const exportWorkout = (workout, options = {}) => {
    ...
    const { format = "pdf", fallbackToText = true } = options;
    ...
    try {
      ...
      if (fallbackToText && format !== "text") {
        ...
        try {
          ...
          console.error(
            "PDFExport: Fallback export also failed:",
            fallbackError
          );
