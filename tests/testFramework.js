  ...
  const assert = {
        ...
        if (expectedError) {
          if (typeof expectedError === "string") {
            if (!error.message.includes(expectedError)) {
              throw new AssertionError(
                `${message}. Expected error message to contain "${expectedError}", got "${error.message}"`
              );
            }
    ...
    contains(array, value, message = "Array does not contain expected value") {
      ...
      if (!array.includes(value)) {
        throw new AssertionError(
          `${message}. Array: ${JSON.stringify(array)}, Value: ${value}`
        );
      }
    },
    ...
    hasLength(
      array,
      expectedLength,
      message = "Array does not have expected length"
    ) {
      if (!Array.isArray(array)) {
      ...
      if (array.length !== expectedLength) {
        throw new AssertionError(
          `${message}. Expected: ${expectedLength}, Actual: ${array.length}`
        );
      }
    },

    /**
     * Assert equality (alias for strictEqual for compatibility)
     */
    ...
    /**
     * Assert that object has a specific property
     */
    hasProperty(
      obj,
      property,
      message = "Object does not have expected property"
    ) {
    ...
    /**
     * Alias for isTrue (for compatibility)
     */
  ...
  async function runAllTests(options = {}) {
    ...
    try {
      ...
      for (const [suiteName, suite] of testSuites) {
        ...
        console.log(
          `TestFramework: Running suite "${suiteName}" with ${suite.tests.length} tests`
        );
      ...
      console.log(
        `TestFramework: Test execution complete. ${results.passed}/${results.total} passed`
      );
    } finally {
      isRunning = false;
    }
